...
import {IngredientPropsType} from "../../types";


const IngredientBadge = ({name, removable, removed, removeIngredient, cancelRemoveIngredient}: IngredientPropsType) => {
    return (
        <div>
            <span id="badge-dismiss-default"
                  style={removed ? {textDecoration: 'line-through'} : {}}
            ...
            {name}
                ...
                {
                    removable ?
                        <button type="button" onClick={removed ? () => cancelRemoveIngredient(name.toLowerCase()) : () => removeIngredient(name.toLowerCase())}
